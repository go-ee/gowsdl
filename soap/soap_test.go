...
func TestClient_Call(t *testing.T) {
	...
	reply := &PingResponse{}
	if err := client.Call("GetData", req, nil, reply, nil); err != nil {
...
func TestClient_Send_Correct_Headers(t *testing.T) {
	...
	for _, test := range tests {
		...
		req := struct{}{}
		reply := struct{}{}
		client.Call(test.action, req, nil, reply, nil)
...
func TestClient_Attachments_WithAttachmentResponse(t *testing.T) {
	...
	if err := client.CallContextWithAttachmentsAndFaultDetail(context.TODO(), "''", req,
		nil, reply, nil, &retAttachments, nil); err != nil {
		t.Fatalf("couln't call service: %v", err)
	}
...
%v
			...
			if err := client.CallWithFaultDetail("GetData", req, nil, &reply, &fault, nil); err != nil {
...
func TestHTTPError(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			defer ts.Close()
			client := NewClient(ts.URL)
			gotErr := client.Call("GetData", &Ping{}, nil, &PingResponse{}, nil)
			if test.wantErr {
				if gotErr == nil {
