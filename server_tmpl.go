...
type SOAPBodyRequest struct {
	...
	{{range .}}
		{{range .Operations}}
				...
				{{$requestTypeName := findTypeName .Input.Message }} ` + `
  				{{$requestTypeName}} *{{$requestType}} ` + "`" + `xml:,omitempty` + "`" + `
		{{end}}
	{{end}}
}
...
{{range .}}
	{{range .Operations}}
		...
		{{$requestTypeName := findTypeName .Input.Message }} ` + `
			{{$requestTypeName}} *{{$responseType}} ` + "`" + `xml:",omitempty"` + "`" + `
	{{end}}
{{end}}
...
{{range .}}
	{{range .Operations}}
		...
		{{$requestTypeName := findTypeName .Input.Message }}
		{{$requestType := findType .Input.Message }}
		{{$requestTypeSource := findType .Input.Message }}
func (service *SOAPBodyRequest) {{$requestTypeName}}Func(request *{{$requestType}}) (*{{$responseType}}, error) {
